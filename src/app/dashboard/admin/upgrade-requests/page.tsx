...
import { Briefcase, CheckCircle2, XCircle, Clock, User, Mail, Phone, MessageSquare, ArrowLeft } from 'lucide-react'
...
export default async function UpgradeRequestsPage({
  ...
  const statusCfg: Record<string, { label: string; classes: string; icon: React.ElementType }> = {
    pending:  { label: 'Pending',  classes: 'bg-warning/10 text-warning',       icon: Clock       },
    approved: { label: 'Approved', classes: 'bg-success/10 text-success',       icon: CheckCircle2 },
    ...
    <div className="space-y-8 max-w-4xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-heading font-bold text-foreground flex items-center gap-2">
            <Briefcase className="h-6 w-6 text-primary" /> Business Upgrade Requests
          </h1>
          <p className="text-muted-foreground text-sm mt-1">
            Review and approve customer requests to become business owners.
          </p>
        </div>
      </div>
      ...
      {params.success === 'approved' && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-success/10 border border-success/20 text-success text-sm font-medium">
          ...
          Request approved! The user&apos;s account has been upgraded to Business Owner.
        </div>
      )}
      {params.success === 'rejected' && (
        <div className="flex items-center gap-3 p-4 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-sm font-medium">
          ...
          Request has been rejected and the applicant has been notified.
        </div>
      )}
      {params.error && (
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 pt-2 border-t border-border">
                      ...
                      <form action={approveUpgradeRequest} className="flex-1 space-y-2">
                        ...
                        <textarea
                          name="admin_note"
                          rows={2}
                          placeholder="Optional note to applicant (e.g. Welcome to Pesanaja.Lab!)"
                        ...
                        <button
                          type="submit"
                          className="w-full flex items-center justify-center gap-2 py-2 px-4 bg-success text-white text-sm font-semibold rounded-lg hover:bg-success/90 transition-colors"
                        >
                          <CheckCircle2 className="h-4 w-4" /> Approve & Upgrade Role
                        </button>
                      </form>
                      ...
                      <form action={rejectUpgradeRequest} className="flex-1 space-y-2">
                        ...
                        <textarea
                          name="admin_note"
                          rows={2}
                          placeholder="Reason for rejection (required for good UX)"
                        ...
                        <button
                          type="submit"
                          className="w-full flex items-center justify-center gap-2 py-2 px-4 bg-destructive/10 text-destructive border border-destructive/20 text-sm font-semibold rounded-lg hover:bg-destructive/20 transition-colors"
                        >
